...
# XXXXXXXXXXXXXXXXXX SCRATCH HERE DOWN


# download thread count. We conject is invariably equal to 1 as is airplane mode
ios_sparse = ios_df.isna().mean().sort_values()
...
sns.barplot(android_df2['is_airplane_mode_a'])
...
# Scratch from here downwards - try to remember to delete this before final submission. ;-)

# zipf = 'P:/ooklabak\\android_2019-12-17.zip'
# android_2019-12-17.zip


# I'm expecting about half a million Android and 1.25m iOS records ... Not getting it!
# stepping through the iOS code, first ook has 6069 records. I'm expecting 113974
...
# rename the Operating System values 'a' and 'i' to 'Android' and 'iOperating System'
ooklameta['Operating System'] = ooklameta['Operating System'].apply(lambda x: 'Android' if x == 'a' else 'iOS')

# data is missing for iOS on 15 April so drop the equivalent Android record
# ooklameta[ooklameta['filename'] == 'p:/ookla/android_2020-04-15.csv']
ooklameta.drop(ooklameta[ooklameta.filename == 'p:/ookla/android_2020-04-15.csv'].index, inplace=True)

# Check the correlation between each OS. Needs a 'wide' view so pipe the output of .pivot to .corr.
